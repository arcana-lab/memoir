...
void ObjectLowering::BasicBlockTransformer(
  ...
  auto mallocFTY =
      llvm::FunctionType::get(i8StarTy, ArrayRef<Type *>({ int64Ty }), false);
  auto mallocf = M.getOrInsertFunction("malloc", mallocFTY);
  auto freeFTY =
      llvm::FunctionType::get(voidTy, ArrayRef<Type *>({ i8StarTy }), false);
  ...
  for (auto &ins : *bb) {
    ...
    if (auto phi = dyn_cast<PHINode>(&ins)) {
      ...
      if (!isObjectIRCall(calleeName)) {
        ...
        // if the return type isn't Object*, then we assume it is an intrinsic
        ...
        switch (calleeCategory) {
          case BUILD_OBJECT: {
            // create malloc based on the object's LLVMRepresentation ; bitcast
          ...
          case READ_POINTER: {
            // create gep. load i8* from the gep. bitcast the load to a ptr to
            // LLVMRepresentation
            std::set<PHINode *> visited;
            auto fieldWrapper =
                parser->parseFieldWrapperChain(callIns->getArgOperand(0),
            ...
            // the pointsTo must be an ObjectType, which we can use to get the
          ...
          case WRITE_POINTER: {
            // create gep. bitcast the value-to-be-written into i8*. store the
            // bitcast
            std::set<PHINode *> visited;
            auto fieldWrapper =
                parser->parseFieldWrapperChain(callIns->getArgOperand(0),
            ...
            if (replacementMapping.find(new_val) == replacementMapping.end()) {
              errs()
                  << "BBtransform: no replacement found for value: " << callIns
                  << "\n";
              assert(false);
            }
      ...
      if (replacementMapping.find(not_the_null_one)
        ...
        errs()
            << "Comparing object* but fail to find it in replacement mapping";
        assert(false);
      }
...
Value *ObjectLowering::CreateGEPFromFieldWrapper(
  ...
  auto llvmPtrType = PointerType::getUnqual(llvmType);
  auto gep = builder.CreateGEP(replacementMapping.at(fieldWrapper->baseObjPtr),
                               indices);
  return gep;
}
