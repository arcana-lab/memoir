...
void TypeMapTy::finishType(StructType *DTy,
                           StructType *STy,
...
Type *TypeMapTy::get(Type *Ty, SmallPtrSet<StructType *, 8> &Visited) {
  ...
  switch (Ty->getTypeID()) {
    ...
    case Type::PointerTyID:
      return *Entry =
                 PointerType::get(ElementTypes[0],
    ...
    case Type::StructTyID: {
      ...
      if (IsUniqued)
        return *Entry =
                   StructType::get(Ty->getContext(), ElementTypes, IsPacked);
...
public:
  IRLinker(Module &DstM,
           MDMapT &SharedMDs,
           IRMover::IdentifiedStructTypeSet &Set,
           std::unique_ptr<Module> SrcM,
    ...
    : DstM(DstM),
      ...
      TypeMap(Set),
      GValMaterializer(*this),
      LValMaterializer(*this),
      SharedMDs(SharedMDs),
      ...
      Mapper(ValueMap,
             ...
             &GValMaterializer),
      IndirectSymbolMCID(
          Mapper.registerAlternateMappingContext(IndirectSymbolValueMap,
                                                 &LValMaterializer)) {
...
GlobalVariable *IRLinker::copyGlobalVariableProto(const GlobalVariable *SGVar) {
  ...
  GlobalVariable *NewDGV =
      new GlobalVariable(DstM,
                         TypeMap.get(SGVar->getValueType()),
                         SGVar->isConstant(),
...
AttributeList IRLinker::mapAttributeTypes(LLVMContext &C, AttributeList Attrs) {
  for (unsigned i = 0; i < Attrs.getNumAttrSets(); ++i) {
    for (int AttrIdx = Attribute::FirstTypeAttr;
         AttrIdx <= Attribute::LastTypeAttr;
         AttrIdx++) {
      ...
      if (Attrs.hasAttributeAtIndex(i, TypedAttr)) {
        if (Type *Ty =
          ...
          Attrs = Attrs.replaceAttributeTypeAtIndex(C,
                                                    i,
                                                    TypedAttr,
                                                    TypeMap.get(Ty));
          break;
...
GlobalValue *IRLinker::copyIndirectSymbolProto(const GlobalValue *SGV) {
  ...
  if (auto *GA = dyn_cast<GlobalAlias>(SGV)) {
    auto *DGA = GlobalAlias::create(Ty,
                                    SGV->getAddressSpace(),
                                    GlobalValue::ExternalLinkage,
                                    SGV->getName(),
                                    &DstM);
  ...
  if (auto *GI = dyn_cast<GlobalIFunc>(SGV)) {
    auto *DGI = GlobalIFunc::create(Ty,
                                    SGV->getAddressSpace(),
                                    GlobalValue::ExternalLinkage,
                                    SGV->getName(),
                                    nullptr,
...
GlobalValue *IRLinker::copyGlobalValueProto(const GlobalValue *SGV,
  ...
  if (auto *SGVar = dyn_cast<GlobalVariable>(SGV)) {
    ...
    if (ForDefinition)
      ...
      NewGV = new GlobalVariable(DstM,
                                 TypeMap.get(SGV->getValueType()),
                                 /*isConstant*/ false,
                                 GlobalValue::ExternalLinkage,
                                 /*init*/ nullptr,
                                 SGV->getName(),
...
Expected<Constant *> IRLinker::linkAppendingVarProto(
  ...
  GlobalVariable *NG = new GlobalVariable(DstM,
                                          NewType,
                                          SrcGV->isConstant(),
                                          SrcGV->getLinkage(),
                                          /*init*/ nullptr,
...
Error IRLinker::linkModuleFlagsMetadata() {
  ...
  for (unsigned I = 0, E = SrcModFlags->getNumOperands(); I != E; ++I) {
    ...
    if (DstBehaviorValue == Module::Min || SrcBehaviorValue == Module::Min) {
      ...
      Metadata *FlagOps[] = {
        (DstBehaviorValue != Module::Min ? SrcOp : DstOp)->getOperand(0),
        ID,
    ...
    if (DstBehaviorValue == Module::Max || SrcBehaviorValue == Module::Max) {
      ...
      // value from between the source and destination values.
      Metadata *FlagOps[] = {
        (DstBehaviorValue != Module::Max ? SrcOp : DstOp)->getOperand(0),
        ID,
        (SrcValue->getZExtValue() > DstValue->getZExtValue() ? SrcOp : DstOp)
            ->getOperand(2)
      };
    ...
    switch (SrcBehaviorValue) {
      ...
      case Module::Append: {
        MDTuple *DstValue =
            ensureDistinctOp(cast<MDNode>(DstOp->getOperand(2)));
  ...
  for (auto Idx : Mins) {
    ...
    if (!SeenMin.count(ID)) {
      ...
      Metadata *FlagOps[] = { Op->getOperand(0),
                              ID,
...
StructType *IRMover::IdentifiedStructTypeSet::findNonOpaque(
    ArrayRef<Type *> ETypes,
    bool IsPacked) {
...
Error IRMover::move(std::unique_ptr<Module> Src,
                    ...
                    bool IsPerformingImport) {
  IRLinker TheIRLinker(Composite,
                       SharedMDs,
