...
class ModuleLinker {
  ...
  DenseMap<const Comdat *, std::pair<Comdat::SelectionKind, LinkFrom>>
      ComdatsChosen;
  bool getComdatResult(const Comdat *SrcC,
...
bool ModuleLinker::getComdatLeader(Module &M,
                                   StringRef ComdatName,
                                   const GlobalVariable *&GVar) {
...
bool ModuleLinker::linkIfNeeded(GlobalValue &GV,
  ...
  if (DGV && !GV.hasLocalLinkage() && !GV.hasAppendingLinkage()) {
    ...
    GlobalValue::UnnamedAddr UnnamedAddr =
        GlobalValue::getMinUnnamedAddr(DGV->getUnnamedAddr(),
                                       GV.getUnnamedAddr());
...
bool ModuleLinker::run() {
  ...
  for (GlobalValue *GV : GVToClone) {
    if (auto *Var = dyn_cast<GlobalVariable>(GV)) {
      auto *NewVar = new GlobalVariable(*Var->getParent(),
                                        Var->getValueType(),
                                        Var->isConstant(),
                                        Var->getLinkage(),
                                        Var->getInitializer());
  ...
  if (Error E =
          Mover.move(std::move(SrcM),
                     ValuesToLink.getArrayRef(),
