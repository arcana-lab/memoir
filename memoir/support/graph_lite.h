...
template <typename ContainerType,
          ...
          typename = std::enable_if_t<
              std::is_convertible_v<remove_cv_ref_t<ValueType>,
...
auto find(std::vector<std::pair<ValueType, T>> &c, const ValueType &v) {
  return std::find_if(c.begin(), c.end(), [&v](const auto &p) {
    return p.first == v;
  });
}
template <typename ValueType, typename T>
...
int count(const std::vector<std::pair<ValueType, T>> &c, const ValueType &v) {
  return std::count_if(c.begin(), c.end(), [&v](const auto &e) {
    return e.first == v;
  });
}
...
std::enable_if_t<std::is_convertible_v<ValueType, FullValueType>, int> count(
    const std::vector<FullValueType> &c,
    const ValueType &v) {
...
template <typename ValueType, typename FullValueType>
std::enable_if_t<std::is_convertible_v<ValueType, FullValueType>, int> count(
    const std::list<FullValueType> &c,
    const ValueType &v) {
  return std::count(c.begin(), c.end(), v);
}
...
class Graph : private detail::EdgePropListBase<EdgePropType>,
              public detail::EdgeDirectionBase<Graph<NodeType,
                                                     NodePropType,
                                                     EdgePropType,
                                                     direction,
                                                     multi_edge,
                                                     ...
                                                     logging>,
                                               direction>,
              public detail::NodePropGraphBase<Graph<NodeType,
                                                     NodePropType,
                                                     EdgePropType,
                                                     direction,
                                                     multi_edge,
                                                     ...
                                                     logging>,
                                               NodePropType>,
              public detail::EdgePropGraphBase<Graph<NodeType,
                                                     NodePropType,
                                                     EdgePropType,
                                                     direction,
                                                     multi_edge,
  ...
  friend detail::EdgeDirectionBase<Graph<NodeType,
                                         NodePropType,
                                         EdgePropType,
                                         direction,
                                         multi_edge,
                                         ...
                                         logging>,
                                   direction>;
  friend detail::NodePropGraphBase<Graph<NodeType,
                                         NodePropType,
                                         EdgePropType,
                                         direction,
                                         multi_edge,
                                         ...
                                         logging>,
                                   NodePropType>;
  friend detail::EdgePropGraphBase<Graph<NodeType,
                                         NodePropType,
                                         EdgePropType,
                                         direction,
                                         multi_edge,
  ...
  static_assert(
      std::is_same_v<NodePropType, std::remove_reference_t<NodePropType>>
          and std::is_same_v<EdgePropType,
                             std::remove_reference_t<EdgePropType>>,
      "Property types should not be references");
  static_assert(
      std::is_same_v<NodePropType, std::remove_cv_t<NodePropType>>
  ...
  static_assert(not((neighbors_container_spec == Container::UNORDERED_SET
                     or neighbors_container_spec
                            == Container::UNORDERED_MULTISET
                     or adj_list_spec == Map::UNORDERED_MAP)
                    and !detail::is_std_hashable_v<NodeType>),
  ...
  static_assert(not((neighbors_container_spec == Container::SET
                    ...
                    and !detail::is_comparable_v<NodeType>),
  ...
  static_assert(
      not((neighbors_container_spec == Container::MULTISET
           or neighbors_container_spec == Container::UNORDERED_MULTISET)
          and multi_edge == MultiEdge::DISALLOWED),
...
private: // type gymnastics
  ...
  using NeighborType =
      std::conditional_t<std::is_void_v<EdgePropType>,
                         NodeType,
...
public:
  using NeighborsContainerType = typename ContainerGen<neighbors_container_spec,
                                                       NodeType,
                                                       EdgePropType>::type;
...
private:
  ...
  static constexpr bool need_pair_iter =
      has_edge_prop
      and (neighbors_container_spec == Container::VEC
           or neighbors_container_spec == Container::LIST);
...
public:
  ...
  Iterator begin() noexcept {
    return Iter<false>(adj_list.begin());
  }
  Iterator end() noexcept {
    return Iter<false>(adj_list.end());
  }
  ConstIterator begin() const noexcept {
    return Iter<true>(adj_list.cbegin());
  }
  ConstIterator end() const noexcept {
    return Iter<true>(adj_list.cend());
  }
...
private: // neighbor access helpers
  ...
  [[nodiscard]] int count_neighbors_helper(const T &node_iv) const {
    ...
    if (pos == adj_list.end()) {
      ...
      abort();
      // ALASKA_SANITY(false, "counting "+ msg +"-neighbors of a non-existent
      // node");
    }
    if constexpr (is_out) {
  ...
  NeighborsConstView get_neighbors_helper(const T &node_iv) const {
    ...
    if (pos == adj_list.end()) {
      ...
      abort();
      // ALASKA_SANITY(false, "finding "+ msg +"-neighbors of a non-existent
      // node");
    }
...
private: // helpers for node search
  ...
  AdjListIterType find_by_iter_or_by_value(const T &iter_or_val) {
    return detail::const_iter_to_iter(
        adj_list,
        static_cast<const Graph *>(this)->find_by_iter_or_by_value(
            iter_or_val));
  }
  ...
  std::pair<bool, NeighborsConstIterator> find_neighbor_helper(
    ...
    if (src_pos == adj_list.end()) {
      if constexpr (logging == Logging::ALLOWED) {
        print_by_iter_or_by_value(
            std::cerr << "(find_neighbor) source node not found",
            src_iv)
            << "\n";
      }
      abort();
    ...
    if constexpr (std::is_same_v<NodeType, detail::remove_cv_ref_t<V>>) {
      ...
      NeighborsConstIterator tgt_pos =
          detail::container::find(src_neighbors,
                                  NodeType{ std::forward<V>(tgt_identifier) });
      return { tgt_pos != src_neighbors.end(), tgt_pos };
    }
  }
  template <bool is_out, typename U, typename V>
  ...
  int count_edges(const U &source_iv, const V &target_iv) const noexcept {
    ...
    if (src_pos == adj_list.end() or tgt_pos == adj_list.end()) {
      if constexpr (logging == Logging::ALLOWED) {
        if (src_pos == adj_list.end()) {
          print_by_iter_or_by_value(
              std::cerr << "(count_edges) source node not found",
              source_iv)
              << "\n";
        }
        if (tgt_pos == adj_list.end()) {
          print_by_iter_or_by_value(
              std::cerr << "(count_edges) target node not found",
              target_iv)
              << "\n";
        }
      }
...
private: // edge addition helpers
  ...
  auto insert_edge_prop(EPT &&...prop) {
    ...
    return EdgePropIterWrap<EdgePropType>{ std::prev(
        this->edge_prop_list.end()) }; // iterator to the last element
  }

  bool check_edge_dup(AdjListConstIterType src_pos,
                      const NodeType &src_full,
                      const NodeType &tgt_full) const {
    if constexpr (multi_edge == MultiEdge::DISALLOWED) { // check is needed only
                                                         // when we disallow dup
  ...
  bool check_self_loop(AdjListIterType src_pos,
                       AdjListIterType tgt_pos,
                       const NodeType &src_full) {
...
private: // edge removal helpers
  ...
  NeighborsConstIterator find_tgt_remove_pos(
    ...
    if constexpr (has_edge_prop and multi_edge == MultiEdge::ALLOWED) {
      ...
      if constexpr (neighbors_container_spec == Container::VEC
        ...
        return std::find_if(tgt_neighbors.begin(),
                            tgt_neighbors.end(),
                            prop_finder);
      } else {
  ...
  std::pair<NeighborsConstIterator, NeighborsConstIterator> find_remove_range(
      NeighborsContainerType &neighbors,
      const NodeType &node) {
    static_assert(has_edge_prop);
    if constexpr (neighbors_container_spec == Container::VEC
                  or neighbors_container_spec == Container::LIST) {
  ...
  remove_edge(const U &source_iv, const V &target_iv) noexcept {
    ...
    if constexpr (multi_edge == MultiEdge::DISALLOWED) { // remove at most 1
      ...
      static_assert(neighbors_container_spec != Container::SET
                    and neighbors_container_spec != Container::UNORDERED_SET);
...
public: // node removal
  ...
  int remove_nodes(const T &node_iv) noexcept {
    ...
    if (pos == adj_list.end()) { // no-op if not found
      if constexpr (logging == Logging::ALLOWED) {
        print_by_iter_or_by_value(
            std::cerr << "(remove_nodes) removing non-existent node",
            node_iv)
            << "\n";
      }
      return 0;
