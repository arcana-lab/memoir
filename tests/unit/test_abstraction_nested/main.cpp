...
Type *outerType = defineStructType("Bar", 2, innerType, UInt64Type());

int main() {
  Object *outerObject = allocateStruct(&outerType);
  Field *field1 = getStructField(&outerType, outerObject, 0);
  Object *innerObject = readStruct(field1);
  Field *field11 = getStructField(&innerType, innerObject, 0);
  writeUInt64(field11, 10);
  Field *field12 = getStructField(&innerType, innerObject, 1);
  writeUInt64(field12, 20);

  Field *field2 = getStructField(&outerType, outerObject, 1);
