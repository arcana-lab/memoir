// Note: this is benchmark #5 which is 2nd after the top C++
// benchmark #7. However, #7 relies on TBB which seems to
// cause trouble on other platforms (it is 2.5x slower than
// #5 on the M1). Also note that #5 (and #7) parallelize
// inside the iterations for a specific depth unlike most
// implementations of the benchmark and can thus have
// outsized performance if the number of cores gets larger
// than about 8 as the implementations are otherwise limited
// by the parallelism in the depth steps (about 10 for depth
// 21).

/* The Computer Language Benchmarks Game
 * https://salsa.debian.org/benchmarksgame-team/benchmarksgame/
 *
 * contributed by Danial Klimkin (C++)
 * contributed by the Rust Project Developers (Rust)
 * contributed by TeXitoi (Rust)
 * contributed by Cristi Cobzarenco (Rust)
 * contributed by Matt Brubeck (Rust)
 * contributed by Dmytro Ovdiienko
 *
 */

#include <algorithm>
#include <atomic>
#include <functional>
#include <iostream>
#include <memory_resource>
#include <numeric>
#include <thread>

/*
using MemoryPool = std::pmr::monotonic_buffer_resource;
*/

/*
struct Node
{
    Node *l, *r;

    int check() const
    {
        if (l)
            return l->check() + 1 + r->check();
        else
            return 1;
    }
};
*/
Type *Node = nameObjectType(
    "Node",
    2,
    getPointerType(getNamedType("Node")), // l
    getPointerType(getNamedType("Node"))  // r
);

int check(Object *node) {
  assertType(Node, node);

  auto node_l_field = getObjectField(node, 0);
  auto node_l = readPointer(node_l_field);

  if (node_l) {
    auto node_r_fld = getObjectField(node, 1);
    auto node_r = readPointer(node_r_field);

    return check(node_l) + 1 + check(node_r);
  } else {
    return 1;
  }
}

/*
Node *make(const int d, MemoryPool &store) {
  Node *root =
      static_cast<Node *>(store.allocate(sizeof(Node)));
  if (d > 0) {
    root->l = make(d - 1, store);
    root->r = make(d - 1, store);
  } else {
    root->l = root->r = nullptr;
  }
  return root;
}
*/

Object *make(int d) {
  setReturnType(Node);

  auto root = buildObject(Node);

  if (d > 0) {
    auto root_l_field = getObjectField(root, 0);
    auto root_r_field = getObjectField(root, 1);
    
    writePointer(root_l_field, make(d - 1));
    writePointer(root_r_field, make(d - 1));
  } else {
    auto root_l_field = getObjectField(root, 0);
    auto root_r_field = getObjectField(root, 1);

    writePointer(root_l_field, nullptr);
    writePointer(root_r_field, nullptr);
  }

  return root;
}

int run_parallel(unsigned depth,
                 int iterations,
                 unsigned int workers =
                     std::thread::hardware_concurrency()) {
  std::vector<std::thread> threads;
  threads.reserve(workers);

  std::atomic_int counter = iterations;
  std::atomic_int output = 0;

  for (unsigned i = 0; i < workers; ++i) {
    threads.push_back(
        std::thread([&counter, depth, &output] {
          std::pmr::unsynchronized_pool_resource upperPool;
          MemoryPool pool{ &upperPool };
          int checksum = 0;

          while (--counter >= 0) {
            Node *a = make(depth, pool);
            checksum += a->check();
            pool.release();
          }

          output += checksum;
        }));
  }

  for (unsigned i = 0; i < workers; ++i) {
    threads[i].join();
  }

  return output;
}

constexpr auto MIN_DEPTH = 4;

int main(int argc, char *argv[]) {
  const int max_depth =
      std::max(MIN_DEPTH + 2,
               (argc == 2 ? atoi(argv[1]) : 21));
  const int stretch_depth = max_depth + 1;

  // Alloc then dealloc stretchdepth tree.
  {
    MemoryPool store;

    Node *c = make(stretch_depth, store);
    std::cout << "stretch tree of depth " << stretch_depth
              << "\t "
              << "check: " << c->check() << std::endl;
  }

  MemoryPool long_lived_store;
  Node *long_lived_tree = make(max_depth, long_lived_store);

  for (int d = MIN_DEPTH; d <= max_depth; d += 2) {
    const int iterations = 1 << (max_depth - d + MIN_DEPTH);
    auto const c = run_parallel(d, iterations);

    std::cout << iterations << "\t trees of depth " << d
              << "\t check: " << c << "\n";
  }

  std::cout
      << "long lived tree of depth " << max_depth << "\t "
      << "check: " << (long_lived_tree->check()) << "\n";

  return 0;
}
