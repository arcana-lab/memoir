...
public:
  ...
  CountingIterator() : val{} {}
  CountingIterator(int v) : val{ v } {}
  ...
  CountingIterator operator+(int v) const {
    return CountingIterator{ val + v };
  }
  int operator-(CountingIterator const &rhs) const {
    return val - rhs.val;
  }
  int operator[](int x) {
    return val + x;
...
int main(int argc, char *argv[]) {
  ...
  std::for_each(
      ...
      [max_depth](auto &res) {
        ...
        res.second = std::transform_reduce(
            std::execution::par,
            CountingIterator(0),
            CountingIterator(iters),
            0,
            std::plus<>{},
            [d](int) {
  ...
  for (const auto &[d, c] : results) {
    std::cout << (1 << (max_depth - d + MIN_DEPTH)) << "\t trees of depth " << d
              << "\t check: " << c << "\n";
  }
