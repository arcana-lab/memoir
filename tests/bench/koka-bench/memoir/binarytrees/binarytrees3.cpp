// Note: this is C++ #3 program coming in at spot 1.9 (relative to the top entry
// as C++ #7). We use this one as both of the top 2 entries (#5 and #7) use
// different kind of parallelism with parallel execution within the iterations
// of sum-count. (it would be interesting to also create a Koka version of this
// but it requires a work-stealing implementation). Even #3 is a bit too
// specialized as it uses the Boost object pools for allocation which frees all
// objects at once instead of freeing per node (like Koka and the other
// languages do).
 ...
 * Jeff Wofford: replaced omp dependency with C++11 threading and load-based job
 * distribution. *reset*
 */
...
Node *make(int d, NodePool &store) {
  if (d > 0)
    return store.construct(make(d - 1, store), make(d - 1, store));
...
int main(int argc, char *argv[]) {
  ...
  const auto work = [&]() {
    while (true) {
      ...
      sprintf(outputstr.get() + LINE_SIZE * d,
